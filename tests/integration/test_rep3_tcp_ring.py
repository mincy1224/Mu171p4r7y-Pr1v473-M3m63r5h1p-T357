...
def _party_worker(party_id: int, party_name: str,
    ...
    try:
        # 1. Bind/listen on ch_prev (non-blocking)
        ...
        # 4. Run ShrRep3(ell=1) then ShrRep3(ell=4)
        t0 = time.monotonic()
...
def run_tests(small=False):
    ...
    # -- Staggered startup: steward → peer0 → peer1 ---
    ...
    if not small:
        # -- Reverse order: peer1 → peer0 → steward ----
        ...
        # -- Peer-first: peer0 → steward → peer1 ------
        ...
        # -- Multi-round: same ring, multiple ell values
