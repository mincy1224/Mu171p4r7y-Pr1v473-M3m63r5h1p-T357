...
def run_tests(small=False):
    ...
    for ell in ells:
        ...
        # ——— share_scalar / recv_scalar_share ————————
        ...
        # ——— recv_data rejects non-bytearray ——————————
        def bad_buf_fn(pid, ch):
            ...
            if pid == 0:
                ...
                try:
                    inst.recv_data(b'\x00' * 4)  # bytes, not bytearray
                    return "no_error"
                except (ValueError, RuntimeError):
                    return "ok"
            return None
        ...
        # recv_key_share rejects non-16-byte buf
        ...
        # recv_key_share rejects non-bytearray
