...
def run_tests(small=False):
    ...
    for ell in ells:
        ...
        # ——— share_scalar / recv_scalar_share / reveal_scalar ———
        ...
        # ——— share_vector / recv_vector_share / reveal_vector ———
        ...
        # ——— flush before any I/O (no-op, no crash) ——
        ...
        # ——— recv_vector_share rejects zero-size ——
        ...
        # ——— reshare_vector alias rejection ———————
        if not small:
            def alias_reshare_fn(pid, prev, nxt):
                inst = mpmt.ShrRep3(ell, pid)(prev, nxt)
                sv = SV(8)
                try:
                    # aliasing vec == sv.thisShare should throw
    ...
    # ring_conv reject invalid ell_to
    try:
